 ...
 * RegExp对象方法
 */
var RegexTestForRegExp = (function () {
    function RegexTestForRegExp() {
    }
    /**
     * test()方法：该方法用于检测一个字符串是否匹配某个模式；
     * @param(参数) str是需要检测的字符串；
     * @return (返回值) 如果字符串str中含有与RegExpObject匹配的文本的话，返回true，否则返回false；
     */
    RegexTestForRegExp.testTest = function () {
     ...
     * 该方法还返回2个属性，index属性声明的是匹配文本的第一个字符的位置；
     * input属性则存放的是被检索的字符串string；
     * 该方法如果不是全局的话，返回的数组与match()方法返回的数组是相同的。
     */
    RegexTestForRegExp.execTest = function () {
        var str = "longen and yunxi";
    ...
    return RegexTestForRegExp;
}());
__reflect(RegexTestForRegExp.prototype, "RegexTestForRegExp");
//# sourceMappingURL=RegexTestForRegExp.js.map